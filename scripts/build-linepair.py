...
dest = root / 'public/linepair/artwork'
...
for name, word in [('titlecase','Linepair'),('lowercase','linepair'),('uppercase','LINEPAIR'),('initials','LP')]:
...
<g fill="#0000a8" transform="scale(1 -1)">{paths}</g>
    ...
    (dest / f'linepair-{name}.svg').write_text(svg, encoding='utf-8')
...
type_at('CANDIDATE LAUNCH NAME / 01', (70,60), 26)
size = 330
while draw.textlength('Linepair', font=ImageFont.truetype(str(source),size)) > 1460:
    size -= 1
type_at('Linepair', (70,270), size)
...
im.save(dest / 'linepair-study.png')
