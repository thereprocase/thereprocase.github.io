"""Build Fillaprint artwork from the released Fillaprint outlines."""
...
<title id="title">{html.escape(word)} — wordmark in Fillaprint</title>
<desc>Unmodified glyph outlines, shaped with the released font's kerning. Fillaprint release artwork.</desc>
...
type_at('FILLAPRINT / V0.1.0 BETA', (70,60), 26)
size = 330
...
type_at('Set in Fillaprint', (70,808), 25)
im.save(dest / f'{slug}-study.png')
(dest / 'provenance.json').write_text(json.dumps({
    'status':'Released Fillaprint artwork',
    'candidate':candidate,
    'font':'Fillaprint', 'version':font['name'].getDebugName(5),
